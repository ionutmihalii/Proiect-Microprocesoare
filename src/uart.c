...
void UART0_Init(uint32_t baud_rate)
{
	...
	/*seteaza bitii de Transmitter Enable si Receiver Enable pe 0*/
	
	UART0->C2 &= ~((UART0_C2_RE_MASK) | (UART0_C2_TE_MASK)); 
	
	/*Configurare Baud Rate
	Modificarile trebuie facute doar atunci cand transmitter si receiver sunt ambele disabled
	...
	*/
	osr = 31; /* Over-Sampling Rate (numarul de esantioane luate per bit-time) */
	
	sbr = (uint16_t)((DEFAULT_SYSTEM_CLOCK)/(baud_rate * (osr+1))); /* Baud Rate Modulo Divisor */	
	sbr++;
